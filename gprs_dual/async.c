 ...
 *
 */
#ident "@(#) head:$RCSfile: async.c,v $	$Revision: 1.3 $"

/****************************************************************************
/* Based of R3 version 5.14 */
...
#define TRACE_MSG_FILE "/usr/taxi/run/traces/gprs_log"
     ...
     CHAR **argv;
{
  ...
  if ((outb_fd = open(TO_UDP_PIPE, O_RDWR)) < 0)
    fprintf(stderr,"pipe open failure - to async errno %d\n",errno);

  if ((inb_fd = open(FROM_UDP_PIPE, O_RDWR)) < 0)
...
#define MAX_SPACES  56
   ...
   for (sq[0] = 0, sq[1] = 0;;)
   {
     ...
     itm_type = 0;
      ...
      if ( status == 0 ) // timeout while waiting for message
	{
	  // Check pkt_id table
	  PktIDCheck();
	  continue;
	 ...
	 if ((alc_ptr->trace == 'Y') &&
	      ...
	      (strncmp("ALL", alc_ptr->trace_mid, 3) == 0)))
	   {
		 ...
		 switch (Tmsg->p_fields[0])
			 ...
			 {
			 case '0':
			   if ( !strncmp( Tmsg->text, ".SP99997", 8 ) )
			     sprintf(DescriptBuff," TPAK_DEFOBJ_NVRAM");
			   else if ( !strncmp( Tmsg->text, ".SP99995", 8 ) )
			     sprintf(DescriptBuff, " TPAK_DEFOBJ_NVRAM");			   
			   else if ( !strncmp( Tmsg->text, ".SP99996", 8 ) )
			     sprintf(DescriptBuff, " TPAK_DEFOBJ_NVRAM");
			   else if ( !strncmp( Tmsg->text, ".SP99998", 8 ) )
			     sprintf(DescriptBuff, " TPAK_DEFOBJ_NVRAM");
			   else if ( !strncmp( Tmsg->text, ".SP99999", 8 ) )
			     sprintf(DescriptBuff, " TPAK_DEFOBJ_NVRAM");			   
			   else
	 ...
	 for ( i = 0; i < MAX_LINKS; i++ )
	   {
         ...
         Rxmsg.persist = 'S'; //link_type;

         // Trap MSG_FROM_DRV messages 97/98 and redirect to MPK Bridge
	 if ( Rxmsg.type == '2' && Rxmsg.p_fields[0] == '1' && Rxmsg.p_fields[1] == '3' &&
	      ((!strncmp(Rxmsg.text, "097", 3)) || (!strncmp(Rxmsg.text, "099", 3)) || (!strncmp(Rxmsg.text, "098", 3))) )
	   {
	     bzero(udp_buf, sizeof(udp_buf));
	     sprintf(udp_buf, "%.5s!%.3s", Rxmsg.route_id, Rxmsg.text);
	     bzero((char *)&serv_addr, sizeof(serv_addr));
	     serv_addr.sin_family = AF_INET;
	     hp = gethostbyname("patrick"); //"ws226.taksihelsinki.fi");
	     if ( hp != NULL )
	       {
		 ...
		 if ( ( sockfd = socket(AF_INET, SOCK_DGRAM, 0 ) ) != 0 )
		   {
		     ...
		     if ( bind(sockfd, (struct socaddr *) &cli_addr, sizeof(cli_addr) ) >= 0 )
...
no_xmit:
	 ...
	 // Clean-up the IP table periodically
	 if ( (alc_ptr->tx.num_tx_msgs[0] % 10) == 0 )
	   veh_purge();
	 break;
   ...
   char *msg;
{
   ...
   tdelay.tv_sec = 5;
   tdelay.tv_usec = 0;
   nfound = select(outb_fd + 1, &readmask, (fd_set *) 0, (fd_set *) 0, &tdelay);

   if ( nfound > 0  && FD_ISSET(outb_fd, &readmask) ) // got a message to read
     {
     ...
     {
       return 0;  // timer expired while awaiting message. Check pktid table here.
     }
}				/* end get_outb_msg */
...
int PktIDCheck()
{
  ...
  for ( i = 0; i < 256; i++ )
    {
      if ( ( alc_ptr->pktid_table[i].sentTime > 0 ) &&
	   ( nowtime - alc_ptr->pktid_table[i].sentTime > 5 ) ) // packetID pending for more than 5 seconds
	{
	  bzero( &LinkPkt, sizeof(LINK_PKT) );

	  // All retries exhausted. Clear entry in table and return NAK to TaxiPak
	  if ( alc_ptr->pktid_table[i].retry <= 0 )
	    {
	      alc_ptr->pktid_table[i].sentTime = 0;
	      alc_ptr->pktid_table[i].retry = 0;
	      alc_ptr->pktid_table[i].type = '';
	      bzero(alc_ptr->pktid_table[i].MMPText, BUFSIZ);
	      bzero(alc_ptr->pktid_table[i].VehIP, 32);

	      bzero( &Nak, sizeof(MSG_QUEUE_HDR) );	      
	      sprintf( MID, "%05x", alc_ptr->pktid_table[i].mid );
	      MID[ROUTE_ID_SIZE] = '\0';
	      memcpy( Nak.route_id, MID, ROUTE_ID_SIZE );
	      sprintf( PKT_ID, "%02x", i );
	      memcpy( Nak.pkt_id, PKT_ID, 2 );

	      Nak.type = '4';
	      Nak.size = 19;
	      memcpy(Nak.p_fields, "G4000", 5);
	      Nak.text[0] = CHECK_CHAR;
	      Nak.text[1] = '\0';
	      Nak.persist = 'S';
	      Nak.multicast = ' ';
	      Nak.delvry_prio = 0x30 + 7 + 1;
	      Nak.delvry_disp = ' ';
	      len = 19;
	      
	      write(inb_fd, &Nak, len + sizeof(Nak.size));
	     ...
	     sprintf( MID, "%05X", alc_ptr->pktid_table[i].mid );
	     sprintf( PKT_ID, "%02X", i );	     
	     fprintf(trace_logfile, "8 %s %s 4G4000%20s\n", MID, PKT_ID, "NOACK");
	     fclose(trace_logfile);	     
	      return;
	  ...
	  sprintf( MID, "%05x", alc_ptr->pktid_table[i].mid );
	  MID[ROUTE_ID_SIZE] = '\0';
	  memcpy( LinkPkt.route_id, MID, ROUTE_ID_SIZE );
	  sprintf( PKT_ID, "%02x", i );
	  memcpy( LinkPkt.pkt_id, PKT_ID, 2 );
	  strcpy(LinkPkt.text, alc_ptr->pktid_table[i].MMPText);
	  
	  LinkPkt.type = '2';
	  LinkPkt.p_fields[0] = '2';
	  if ( alc_ptr->pktid_table[i].type == 'O' )
	    LinkPkt.p_fields[1] = '1';
	  else
	    LinkPkt.p_fields[1] = '2';

	  len = 2 + SEQ_NO_SIZE + PKT_LEN_SIZE + ROUTE_ID_SIZE + PKT_ID_SIZE + 5 + P_SIZE + strlen(alc_ptr->pktid_table[i].MMPText);
	  udp_gprs_write( Fd_tx[7], &LinkPkt, len );
	  printf("Retry #%d\n", alc_ptr->pktid_table[i].retry--);
