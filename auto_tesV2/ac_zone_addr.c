...
*  @(#)  $Author: jwelch $
*  @(#)  $Source: /taxi-scan/taxi_proj/cvs/taxi-scan/src/auto_tes/ac_zone_addr.c,v $
*
*  Copyright (c) 2000 - MobileSoft Consulting, Inc. Dublin OH
*
***********************************************************************/
#ident "@(#) head:$RCSfile: ac_zone_addr.c,v $	$Revision: 1.3 $"

static char rcsid[] = "$Id: ac_zone_addr.c,v 1.3 2004/02/02 18:55:53 jwelch Exp $";
...
int *zip;
{
	...
	rc = read_address(fleet, pickup,zip);
	...
	*(end+1) = 0;
    	...
    	if (db(STRNAME_FILE_ID,READ_KEYS,&sn_key2,ISEQUAL,0) >= 0)
	  {
		...
		if ((s_idx != ad_ptr->str_index) || (strncmp(st_predir,ad_ptr->str_pre_dir,2) != 0) ||	/* in range? */
		  ...
		  return(-1);
#else
                if ((s_idx != ad_ptr->str_index) || (strncmp(st_city,ad_ptr->str_city,3) != 0))
                  return(-1);
#endif
