static char sz__FILE__[]="@(#)in_read_fld.c	10.1(ver) Date Release 12/2/94" ;
/****************************************************************************
*                                                                           *
*                           S C C S   I N F O                               *
*                                                                           *
* @(#)  in_read_fld.c; Rel: 10.1.0.0; Get date: 4/18/95 at 16:48:05
* @(#)  Last delta: 12/2/94 at 11:13:18
* @(#)  SCCS File: /taxi/sccs/s.in_read_fld.c
*                                                                           *
 ...
 * read (=field_num), and  a pointer to the buffer which is used to store 
 ...
 *	BLOCK_TIME
 */
int read_field(win_fld, ptr_scr_fld, field_num, read_buf, field_entered,should_clear_ptr)
	...
	int *should_clear_ptr;	/* pointer to flag; 0= clear the field upon entry of 1 st char ; 1= do not clear */
{
	...
	*field_entered = 0;
	 ...
	 */
	ptr_to_fld = ptr_scr_fld + field_num;
	...
	initialize_read_buf(win_fld,ptr_scr_fld,field_num,read_buf,&read_ptr,&end_ptr,&read_count,&old_entry_exists);
	...
	while ( (!end_read_field) && (read_count <= max_chars) )  {
		...
		if(Saw_interupt)  /* if CTRL-X and we start entering data remove this record */
		{
...
right */
			...
			case '\025': /* ^U for clear field */
				entry_changed = 1;
				read_clear_field(win_fld, ptr_scr_fld,field_num); 		/* field; so clear the entry field */
				position = min_position;
...
out,*/
                        read_buf[0] = '\0';                              /* set read_buf[0] = ' '; this happens when the user
*/
			if ( entry_changed )
 ...
 */
initialize_read_buf(win_fld,ptr_scr_fld,field_num,read_buf,ptr_read_ptr,ptr_end_ptr,ptr_count,ptr_old_entry_exists)
	WINDOW *win_fld;	/* window in which read_field() is to operate */
	struct scr_flds *ptr_scr_fld;	/* pointer to array of field_num structures for this window */
	int field_num;	/* field # being read */
	...
	int *ptr_old_entry_exists;	/* does old entry exist */
{
	...
	struct scr_flds *ptr_to_fld;	/* pointer to field_num struct of field being read */
