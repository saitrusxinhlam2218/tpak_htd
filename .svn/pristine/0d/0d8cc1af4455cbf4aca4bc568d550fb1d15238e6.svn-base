...
*  @(#)  $RCSfile: getpar.c,v $
*  @(#)  $Revision: 2.0 $
*  @(#)  $Date: 2012/08/28 14:24:29 $
*  @(#)  $Author: taxiprod $
*  @(#)  $Source: /usr/users/jwelch/cvs_g7/taxi-g7/bin/async/getpar.c,v $
*
...
CHAR	*A_shm_ptr;		/* Pointer to the beginning of raw SHM */
...
/* SHM & SEM will be created if not existant, and SEM value will be set to 
	...
	if (semctl(pm_sal_id, 0, SETVAL, arg) == -1)
	{
		par_errno = PM_ERR_CONFIG;
		return(ERROR);
	}
...
INT sa_attach ()
{
	...
	A_shm_ptr = shmat(pm_sa_id, (char *) 0, (int) shmflag);
	if ((int) A_shm_ptr == ERROR)
		return(ERROR);
 	else
	{
		sts_area = (SHM *) A_shm_ptr;
		return(SUCCESS);
	}
}
