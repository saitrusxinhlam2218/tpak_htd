static char sz__FILE__[]="@(#)msgq.c	10.1(ver) Date Release 12/2/94" ;
 /****************************************************************************
 *                                                                           *
 *                           S C C S   I N F O                               *
 *                                                                           *
 * @(#)  msgq.c; Rel: 10.1.0.0; Get date: 4/18/95 at 16:49:31
 * @(#)  Last delta: 12/2/94 at 11:36:22
 * @(#)  SCCS File: /taxi/sccs/s.msgq.c
 *                                                                           *
 *****************************************************************************/
...
REQUESTDEF  *msgx;
{
    ...
    t=(long)mtype;
    hMsg = (char *)malloc( sizeof( REQUESTDEF ) );
    tmpMsg = ( REQUESTDEF * ) hMsg;

    if ((msgrcv(msgqid, (void *)tmpMsg, sizeof(REQUESTDEF), t, IPC_NOWAIT)) == -1) 
    {
	if (errno==ENOMSG)
	  {
	    free( hMsg );
	...
	else 
	{
      ...
      {
	memcpy( msgx, tmpMsg, sizeof(REQUESTDEF) );
	free( hMsg );
	return(1);
      }
}
