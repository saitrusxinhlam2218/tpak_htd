...
extern int num_tc, free_calls_nbr, free_call_msg_nbr;
int free_lists_nbr = 0, total_lists_nbr = 0, free_elements_nbr = 0, total_elements_nbr = 0;
int num_excepts;
...
LL_debug()
{
  ...
  if ( ( debug_file = fopen("/usr/taxi/run/traces/tpak_dbug", "a" )) == NULL )
    return;
  else
    tmtime = time((time_t) 0 );
  stime = ctime(&tmtime);
  fprintf(debug_file, "%.8s", &stime[11] );
  fprintf( debug_file, " %04d/%04d(C) %04d(L) %04d(N) %04d(EXCPT) %04d(TC) %03d(CL_MSG)\n",
	   4000 - free_calls_nbr, 4000, 
	   total_lists_nbr,
	   total_elements_nbr,
	   num_excepts, num_tc,
	   free_call_msg_nbr );
