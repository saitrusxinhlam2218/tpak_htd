...
*                           S C C S   I N F O                               *
*                                                                           *
* @(#)  dr_common.c; Rel: 10.1.0.0; Get date: 4/18/95 at 16:47:19                      *
  ...
  extern struct fleets *fleet[];
	...
	while(1)   {
		...
		if ( (key_val == ALTERNATE_KEY) && (piu == F_ATTR_1) )
		  {
		    if ( dr_ptr->fleet == 'H' ) /** Helsinki Fleet 'H' is fleet 0 **/
		      attr_select( SELECT_DRIVER_ATTR, 0, dr_ptr->attributes );
		    else
		      attr_select( SELECT_DRIVER_ATTR, dr_ptr->fleet - 'A', dr_ptr->attributes );
		    
                    mvaddstr( D_ATTR_ROW, 0, "                                            ");
...
int piu;
{
	...
	switch(piu)  {
		...
		case F_SEAL_16 :
			...
			break;
		case F_ATTR_1 :
			addstr(catgets(DR_MAINT_m_catd, DR_MAINT_SET, DR_MAINT_35, "Flag indicating driver has attributes "));
			break;
		case F_SP_FM_DATE :
		case F_SP_D_FM_DATE :
		case F_SP_X_FM_DATE :
