static char sz__FILE__[]="@(#)da_autob.c	10.1(ver) Date Release 12/2/94" ;
 /****************************************************************************
 *                                                                           *
 *                           S C C S   I N F O                               *
 *                                                                           *
 * @(#)  da_autob.c; Rel: 5.1.0.0; Get date: 2/27/90 at 12:20:40
 * @(#)  Last delta: 12/14/89 at 17:00:03
 * @(#)  SCCS File: /taxi/sccs/s.da_autob.c
 *                                                                           *
 *****************************************************************************/
...
=====================================================================*/
da_autob(fleet_id,start_date,start_time,start_dt_tm,end_date,end_time,ending_dt_tm,fleet_name)
char fleet_id;
char *start_date;
char *start_time;
long start_dt_tm;
...
char *fleet_name;
{
	...
	int fleetno;
	int report_no=21;
	register int autobook_counter, call_counter;
	...
	while (ccode>=0 && shiftrec->fleet==working_fleet) { 
		...
		if (shiftrec->extended_type[1] == AUTOBOOK_CALL) {
			autobook_counter++;
		}
	...
	sprintf(systembuff,"%s %c %s %s %ld \"%s\" %s %ld \"%s\" %d %d > %s/%s ",
		"sacego -q da_autob",fleet_id,start_date,start_time,start_dt_tm,
		end_date,end_time,ending_dt_tm,fleet_name,autobook_counter,call_counter,REPORT_DATA,report_name);
	system(systembuff);
