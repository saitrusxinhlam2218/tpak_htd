static char sz__FILE__[]="@(#)de_lmain.c	10.1(ver) Date Release 12/2/94" ;
 /****************************************************************************
 *                                                                           *
 *                           S C C S   I N F O                               *
 *                                                                           *
 * @(#)  de_lmain.c; Rel: 5.2.0.0; Get date: 2/27/90 at 12:21:41
 * @(#)  Last delta: 1/16/90 at 17:13:15
 * @(#)  SCCS File: /taxi/sccs/s.de_lmain.c
 *                                                                           *
 *****************************************************************************/
...
/*printf("da_calls_t: output to %s shiftrec->fleet %c \n",report_name,shiftrec->fleet);*/
		...
		while (ccode>=0 && shiftrec->fleet==fleet_id) { 
			...
			if(shiftrec->pri_status == 64 )
				break;
			if((shiftrec->extended_type[0] == TIMEC || shiftrec->extended_type[1] == TIMEC ||
							 shiftrec->extended_type[2] == TIMEC || shiftrec->extended_type[3] == TIMEC ||
			                                 shiftrec->extended_type[0] == SUBS || shiftrec->extended_type[1] == SUBS ||
							 shiftrec->extended_type[2] == SUBS || shiftrec->extended_type[3] == SUBS )
					&& shiftrec->due_date_time < ending_dt_tm && shiftrec->due_date_time > start_dt_tm)
				print_record_1();			/* print the transaction in buf */

			ccode=db(CALL_FILE_ID,READ_KEYS,&cl_key9,ISNEXT,0);
		}	/* end of while 1 */

		print_ending(fleet_id,catgets(REP_catd, 1, 2, "LIVE TIME CALLS"));
...
{
	char title2[50];
	sprintf(title2,catgets(REP_catd, 1, 3, "       LIVE TIME CALLS by call due time"));
...
printf("%s",iobuff);
sprintf(iobuff,"%1c %-2.2s ",
shiftrec->pckup_str_suf, shiftrec->pckup_pre_dir);
printf(iobuff);
sprintf(iobuff," %-20.20s ",
shiftrec->pckup_str_name);
printf(iobuff);
