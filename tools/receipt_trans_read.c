     ...
     char **argv;
{
  ...
  sprintf( filename, "/usr/local/scratch/receipt_trans.%s", argv[1] );
  fp = fopen( filename, "r" );
  if ( fp == NULL )
    exit(-1);
  ...
  while ( !done )
	...
	{
	  pDelimit = NULL;
	  pDelimit = strstr( sMsg, "*" );
	  if ( pDelimit != NULL )
	    {
	      pDelimit = (pDelimit + 2);
	      strcpy( source_buf, pDelimit );
	      Unescape( source_buf );
	      source_buf[strlen(source_buf)-1] = '\0';
	  ...
	  else if ( ( pDelimit = strstr( sMsg, "ITM-UNKNOWN 64" ) ) != NULL )
	    {
